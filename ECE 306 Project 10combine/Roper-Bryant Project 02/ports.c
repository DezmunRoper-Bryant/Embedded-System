...
// P1DIR |= V_A1_SEEED; // Direction = output
//
...
// P1DIR |= V_DETECT_L; // Direction = input              //Change to input  (DONE)
//
...
//------------------------------------------------------------------------------
 ...
 // Need to initalize as gpio (DONE)
 P4SEL0 &= ~UCB1_CS_LCD;
 P4SEL1 &= ~UCB1_CS_LCD;
...
// P6OUT |= LCD_BACKLITE; // Initial Value = Low / Off       Set to on (DONE)

 P6SEL0 |= R_FORWARD; // R_FORWARD GPIO operation
 P6SEL1 &= ~R_FORWARD; // R_FORWARD GPIO operation
 //P6OUT |= R_FORWARD; // Initial Value = On
...
// P6OUT &= ~L_REVERSE; // Initial Value = Off
 //P6OUT |= L_REVERSE; // Initial Value = On
